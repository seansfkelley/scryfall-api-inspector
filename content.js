...
const CARD_UUID_RE = /cards\.scryfall\.io\/[^?#]*\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\.[a-z]+/i;
  ...
  for (const m of mutations) {
    if (m.type === 'childList') {
      m.addedNodes.forEach(node => {
        ...
        else if (node.querySelectorAll) node.querySelectorAll('img').forEach(wrap);
      });
